...
def load_data():
    if not CLEAN_CSV.exists():
        raise FileNotFoundError(f"CSV no encontrado: {CLEAN_CSV}\nEjecuta primero: python -m cleaning.main")
...
def fig_ranking(df):
    ...
    colores = [PALETTE["red"] if c == "LT" else PALETTE["blue"] if c == "ES" else PALETTE["lgray"] for c in paises]
    ...
    ax.set_title("Brecha digital por discapacidad severa — Ranking europeo 2024\n"
                 "Diferencia entre personas sin discapacidad y con discapacidad severa",
...
def fig_genero(df):
    gen = df[(df["disability_level"] == "Severely limited") &
             (df["age_group"] == "Total") &
             (df["sex"].isin(["Female", "Male"]))].copy()
    ...
    ax.set_title("Doble vulnerabilidad: género × discapacidad severa (2024)\n"
                 "España presenta la mayor brecha de género del dataset (9,92 pp)",
                 fontweight="bold", color=PALETTE["navy"])
    ax.legend()
    ax.set_xlim(45, 108)
    fig.text(0.01, -0.03, SOURCE_NOTE, fontsize=8, color=PALETTE["gray"], style="italic")
...
def fig_edad(df):
    es  = df[(df["country_code"] == "ES") & (df["sex"] == "Total") &
    ...
    ax.set_ylim(60, 108)
    ax.set_ylabel("% que usa Internet")
    ax.set_title("España — Efecto del envejecimiento sobre la exclusión digital (2024)\n"
                 "Grupo 55-74 con discapacidad severa: 76,85% (perfil más vulnerable)",
                 fontweight="bold", color=PALETTE["navy"])
    ax.legend(fontsize=9)
    ax.text(0.5, 63, "Nota: datos 16-24 con discapacidad severa no disponibles en Eurostat",
...
def fig_exclusion(df):
    core = df[(df["disability_level"] == "Severely limited") &
              (df["sex"] == "Total") & (df["age_group"] == "Total") &
              (df["is_reliable"]) & (df["country_code"] != "EU27_2020")]
    core = core.sort_values("pct_internet_use")
    core["pct_excluidos"] = 100 - core["pct_internet_use"]
    eu_excl = 100 - 82.29

    colores = [PALETTE["red"] if c == "LT" else PALETTE["blue"] if c == "ES"
               else PALETTE["lgray"] for c in core["country_code"]]
    ...
    for bar, val, code in zip(bars, core["pct_excluidos"], core["country_code"]):
        ax.text(bar.get_x() + bar.get_width()/2, val + 0.4, f"{val:.1f}%",
                ...
                fontweight="bold" if code in ("ES", "LT") else "normal")
    ax.set_ylabel("% excluidos digitalmente")
    ax.set_ylim(0, 48)
    ax.set_title("Exclusión digital en personas con discapacidad severa (2024)\n"
